...
ALTER TABLE documents ADD COLUMN IF NOT EXISTS url_id BIGINT;
...
ALTER TABLE document_versions ADD COLUMN IF NOT EXISTS document_id BIGINT;
...
ALTER TABLE chunks ADD COLUMN IF NOT EXISTS document_version_id BIGINT;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_chunks_document_version_id'
          AND conrelid = 'chunks'::regclass
    ) THEN
        ALTER TABLE chunks
            ADD CONSTRAINT fk_chunks_document_version_id
